...
logger = setup_logger('first_strategy')
...
class FirstStrategy(BaseStrategy):
    def __init__(self, model_preds):
        ...
        """
        self.y0_trmt = float(model_preds['trmt_return_1h_out'][0][0])
        self.y0_ctrl = float(model_preds['ctrl_return_1h_out'][0][0])
        self.y1_trmt = float(model_preds['trmt_sigma_1h_out'][0][0])
        self.y1_ctrl = float(model_preds['ctrl_sigma_1h_out'][0][0])
        self.t = float(model_preds['t_out'][0][0])
    ...
    def print_info(self):
        logger.info(f"FirstStrategy Info: y0_trmt={self.y0_trmt}, y0_ctrl={self.y0_ctrl}, "
                    f"y1_trmt={self.y1_trmt}, y1_ctrl={self.y1_ctrl}, t={self.t}, "
    ...
    def execute_trading_logic(self):
        ...
        if not self.config_retriever():
            logger.error("Configuration retrieval failed. Using default settings.")
    ...
    def config_retriever(self):
        ...
        # Initialize Redis client
        logger.info(self.sharpe_bins)
        return True
    
    def cal_value_tree(self):
        # calculate final score 
        up_score = self.y0_trmt * self.t
        down_score = self.y0_ctrl * (1-self.t)
        self.final_score = self.t
    ...
    def cal_direction(self, mode=1):
        ...
        if self.quantile >= self.top_threshold and self.final_score > 0:
        ...
        elif self.quantile <= self.bottom_threshold and self.final_score < 0:
            ...
            logger.warning(f"Quantile {self.quantile} is not within the trading thresholds. Skipped.")
            if not self.fail_reason:
    ...
    def cal_leverage(self):
        ...
        logger.info(f"Sharpe ratio {sharpe_ratio} exceeds last bin. Leverage set to {self.leverage}.")
        return True
    ...
    def cal_stop_loss_price(self):
        ...
        if self.direction == 'LONG':  # Long position
            ...
            while self.stop_loss_price > self.cur_price:
